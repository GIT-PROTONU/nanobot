...
When building vision: start with one GLES2 fragment shader doing colour-threshold blob tracking on camera frames, publish a `/target` bearing for the ESP32 to chase. Fold the work into `app_hub` (which already owns the camera via `web_control`) rather than a new process — see [[gpu-vision-features-todo]] for the full cost/architecture reasoning.

**Correction, 2026-07-11: the "Cedrus MJPEG-decode → shader → H.264-encode" combined-path idea above is WRONG and should not be revisited.** A live probe of `/dev/video0` (`cedrus`, confirmed built-in/always-active) shows it decodes only `MG2S`/`S264`/`S265`/`VP8F` (MPEG-2/H.264/H.265/VP8 parsed slice data) — **no JPEG/MJPEG in either direction**, and mainline Allwinner VPU support is decode-only anyway (no H.264 encode). See [[gpu-vision-camera-architecture]] for the actual resolved design for running GPU vision and the browser's live view simultaneously (CPU JPEG-encode via `libturbojpeg0`, not the VPU).
