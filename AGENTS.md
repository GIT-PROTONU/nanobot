...
**TODO — map still skews vs the room after manual driving (2026-09-11, UNRESOLVED):** after clearing a fresh map, driving one lap a few metres by hand, then parking, the map walls do NOT line up with the physical room *and* the SLAM pose does not sit back on the wheel chain. This is a **live, open problem** — several candidate causes were found, two big walk-mechanisms were fixed and deployed, but the symptom persisted on the next test, so treat the current `nav_node.py` wheel-authority changes as PARTIAL and keep the diagnosis open. Verified facts and the trail:
...
- **Still-open candidate causes (next things to try):**
  1. Even a 9 cm idle gap is too much — the pose should return to within ~2 cm of odom when parked. Instrument WHICH scan/correction moves the pose last while driving (temporary per-scan log of `trusted / cand / ax,ay / dpos / dhead`), because the gate fix reduced but did not eliminate the final offset.
  2. `loop_alpha 0.1` is too slow for single-drive convergence — try 0.3-0.5 with a matching `loop_apply_thresh`, and/or run the probe more often (`loop_probe_every`).
