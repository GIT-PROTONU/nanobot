...
- **Recovery motions toggle — the no-recovery BT (added 2026-09-23):** the Map view's **Recovery** switch, MIRRORED on the Drive tab's Map (Nav2) card as "Recovery motions" (`#mapRec` + `#navRec`, kept in sync — both POST `GET/POST /nav/config {nav_recovery: bool}`; `nav_recovery_enable` param on web_control, persisted to `nav.json`, boot-restored by `_apply_saved_nav_cfg` with a strict-bool guard) turns the failed-goal recovery branch (clear costmaps → BackUp 0.15 m → Spin 90°, `recovery_bt.xml`'s RecoveryNode) OFF: goals then go out as **NavigateToPose ACTION goals whose `behavior_tree` field points at `config/nav2/no_recovery_bt.xml`** (the same plan+follow PipelineSequence, NO RecoveryNode — a failed plan/follow aborts the goal with NO robot motion at all). WHY the per-goal field and not a param push: Humble's bt_navigator loads `goal->behavior_tree` **on every new goal** (`goalReceived` → `loadBehaviorTree`, empty = the configure-time default), but reads `default_nav_to_pose_bt_xml` ONCE at configure (`getDefaultBTFilepath` → a BtActionServer member) and has NO dynamic-params callback — a runtime SetParameters push would be silently ignored, so the goal field is the only restart-free lever (verified against the Humble sources; all the no-recovery tree's plugins are already in `plugin_lib_names`). Routing: `telemetry.publish_json` consults web_control's `_publish_nav_goal` (wired as the telemetry `_goal_hook` in `__init__`) for every `/goal_pose` POST (map clicks, Locations Go), and `_publish_skill_action` calls it directly for skill go-tos — recovery ON is byte-identical to the old topic path; OFF submits the action goal instead (fire-and-forget `send_goal_async` on a full ActionClient created in `__init__` BEFORE `_apply_saved_nav_cfg`/the HTTP bind; accept/reject logged from the response future). A failed send (XML missing = robot_bringup not rebuilt; nano-nav down) is REPORTED to the page as `{"error": ...}` and **never downgraded to a recovery-BT topic publish** (unit-tested in `test_nav_config.py`; smoke covers the GET/POST/nav.json echo + the strict-bool refusal). Deploy note: the new XML installs via robot_bringup's setup.py glob — **one colcon build (deploy.sh does it) must land on the board** before the OFF state works (landed + gateway-verified on the board 2026-09-23; the physical motion verify is tracked in docs/TODO.md). Two behavior notes: (1) Humble REJECTS a preempting goal that requests a different BT than the one currently running — toggling mid-navigation means the next goal waits for the current one (cancel first); (2) a goal rejected/failed on the no-recovery tree simply returns FAILED — the web chip + Nav log show it like any other failure.
