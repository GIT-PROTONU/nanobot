...
Sensor-heading-vs-drive frame flip (`slam_nav heading_flip`, added 2026-09-11): this unit's sensor head (LDS + IMU) is mounted **facing the robot's back**, so the whole heading frame (IMU yaw, EKF/odom yaw, lidar beam 0) sits 180° from the drive direction. The map is internally **consistent** (scan matcher keeps working, no lost-storms — the relative geometry is identical), but the global "which way is front" anchor is off: the **web map icon's front tick points at the physical back** and **autonomous nav drives the wrong way** (it rotates the physical back toward the goal then drives forward = away). Manual driving is unaffected (never consults the map). Fix = **one knob, three ingest flips**: `slam_nav heading_flip: true` in `robot.yaml` → `nav_node.py` adds π to all three heading references together (`_on_odom` yaw, `_on_euler` yaw, `_on_scan` beam angles). Rotating all three preserves every downstream **difference** (yaw deltas, the `(pth - oth)` map-vs-odom rotation, match/integrate geometry — verified bit-identical map + identical match scores, with only the heading label rotated π), while making `pth` mean the **physical** front heading — which also fixes the front-stop cone (`_ang_abs`), `/slam_pose`/TF orientation, and the web wall-guard (it probes along `_slam_pose` heading). **Restart-only** (a live flip would re-anchor a live map by 180°); default `false` for the usual front-facing mount. The browser's **Lidar hero view** still draws raw `/scan.bin` beam 0 as "the nose" and was NOT flipped — if the head stays back-mounted it will look 180° off versus the now-correct SLAM map icon.
