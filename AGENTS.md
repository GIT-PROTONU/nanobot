...
- **Idle controller (`telemetry.py`, the OWNER of `/lds_target_rpm`)** — a 1 Hz
  ...
  **active** (commanded `/cmd_vel` above `vision_bumper_cmd_eps` within
  ...
  nav status is trusted only within a PER-STATUS window of its last arrival
  (`lds_nav_busy`, 2026-09-23): `navigating`/`canceling` for `LDS_NAV_STALE` (90 s) —
  the status topic is event-driven, so a live goal is really held up by `/cmd_vel`, and
  the age bound stops a mid-navigation `nano-nav` death from freezing "navigating"
  and keeping the lidar awake on a parked robot forever — but **`planning` for the
  much longer `LDS_PLANNING_STALE` (300 s)**: during planning there is NO `/cmd_vel`
  to refresh the motion clock and the planner NEEDS scans before it can compute a
  path (the 2026-09-23 deadlock below), and a slow lidar wake can eat the whole 90 s.
...
### Click-to-goal planning deadlock + Nav log (2026-09-23)

- **The deadlock (found live):** click a goal on the web map while the lidar is
  parked → status sticks on **"planning"** forever, robot never moves. Chain: the
  idle controller parked the lidar (quiet robot) → no scans → slam_toolbox stops
  updating `map→odom` (it keeps REPUBLISHING the transform, but with the stamp of
  the LAST processed scan, so the buffer's map→odom is permanently frozen in time)
  → Nav2's `Time(0)` ("latest") map→base_link lookups resolve the chain to the
  FROZEN map→odom stamp → the odom→base_link half at that old time is pruned from
  tf2's 10 s cache → every planner/costmap lookup fails
  `Exapolation… "Lookup would require extrapolation into the past"` → the planner
  retries forever → never EXECUTING → no `/cmd_vel` → the (then-90 s) nav window
  expired → lidar parked again → repeat. The goal itself is a red herring:
  `_mk_goal` stamps 0 (latest) already. **Fix: `lds_nav_busy` gives `planning` its
  own 300 s trust window** (`LDS_PLANNING_STALE`) so the lidar stays up until scans
...
- **SECOND live finding (2026-09-23, after the deploy restart): a nano-nav restart
  ...
  **Once the costmap has blocked past the query timeout, resuming scans does NOT
  ...
  The clean structural fix (DONE 2026-09-24, live-verified): **the `unit_exec.sh
  nav-loader` LIDAR GATE** — the loader now holds its load until the map frame CAN
  exist (vitals `lds.hz ≥ 2.0` fresh + `nano-slam`/`nano-tf` active), WAKES a parked
  lidar itself via jam-safe gateway POSTs (0 → setpoint every 10 s; a bare 300 after
  the boot park can trip the ESP32's 6 s jam guard and LATCH the motor — the 0-first
  cycle is required, found live), and loads anyway with a warning after `LDS_GATE_SECS`
  (30) so a dead lidar delays boot instead of hanging the target. Activation can never
  start into a dead map frame, the reply never expires, the wedge disappears by
  construction — and the disable-idle-park-then-bounce choreography below is NO LONGER
  NEEDED for bounces. **Same-day bonus fix: the loader's `ros2 launch` can HANG after
  a successful load** (all components loaded + "Managed nodes are active", launch
  lingered 3+ min — zenoh session teardown, same family as the container's stop
  wedge), so it now runs under `timeout -k 10 60` with a linger forced to success
  (Type=oneshot has NO default start timeout — verified live). See docs/TODO.md.
- **Nav log (the visibility half, also the verbose-SLAM-log request):**
  ...
  click/location/skill), `POST /nav/cancel`, `/map/clear`, action-status
  transitions **with durations** (planning→navigating→arrived/failed, terminal
  "goal reached/FAILED in Xs"), the idle controller's wake/park **with its reason**,
  `/cmd_vel` motion start/stop transitions, and a **planning-stuck watchdog**
  (`PLANNING_WARN_SECS` 10, rate-limited 15 s) that names the actual failure mode:
  `/map` stale → "slam needs fresh scans for the map→odom TF"; `/map` fresh →
  "the goal is not being processed — nano-nav may still be activating or was
  restarted". Served via
  **`GET /nav/log?since=<id>`** (incremental, JSON; NOT the SSE frame — the frame
  is a typed contract), mirrored to `journalctl -u nano-app` (`navlog:` prefix).
  The Drive tab's SLAM & navigation group has a **Nav log** card (polls 1 Hz while
  Drive is visible, newest-first, 250-row cap, ⏸ Hold freeze) — the first thing to
  read when "it keeps planning" or a goal goes missing.
...
- **Keep-away zone + robot size — costmap geometry, live-tunable (added 2026-09-23):** the "Navigation pace" card gained two more sliders riding the SAME `GET/POST /nav/config` + `nav.json` persistence: **Keep-away zone** (`nav_inflation_m`, 0..0.6 m, default 0.25) and **Robot size ⌀** (`nav_robot_diam_m`, 0.1..1.0 m, default 0.32 — the web UI works in diameter, the costmap wants radius). They push to BOTH costmap components (`/local_costmap/local_costmap`, `/global_costmap/global_costmap`) as SCALAR SetParameters: `robot_radius` (= ⌀/2, handled by `Costmap2DROS::dynamicParametersCallback` → rebuilds the footprint) and `inflation_layer.inflation_radius` (namespaced with the yaml's plugin name; handled by `InflationLayer::dynamicParametersCallback` → recomputes caches + reinflates) — verified against the Humble sources, **live, no nano-nav restart**. Same lifecycle race as the smoother (the costmap's dynamic callback attaches at `on_activate`; the inflation layer's is live from configure): pushes land on each costmap's OWN `/transition_event` active subscription (per-target rate limit — smoother/local/global activate at staggered moments, a shared limiter would swallow the later pushes) + the bounded boot re-apply (60 s service + 90 s GetState-active wait per target in the same `_reapply_nav_cfg_when_up` thread). The web map's **orange dashed keep-away bubble** + the **red robot footprint circle** draw from `f.nav.inflation`/`f.nav.robot_radius` (telemetry reads web_control's own params each tick, fallback constants `NAV_INFLATION_M`/`NAV_ROBOT_DIAM_M`; smoke test pins the fields). `nav_inflation_m: 0` = no inflation zone (bubble hidden). The new `f.nav.robot_radius` key is additive — the page falls back to 0.16 when absent.

- **ObstacleLayer on BOTH costmaps (added 2026-09-24)** — live `/scan` hits are marked/cleared (raytrace) in the costmaps immediately instead of waiting for slam's ~5 s `/map` repaint + static-layer re-ingest. Fix for "robot crowds an obstacle then stops mid-goal" (docs/TODO.md NEW 2026-09-24, live-confirmed 06:47 that morning: RPP "collision ahead" → patience exceeded → goal FAILED 12.5 s in, ~9 s in for the first attempt): NavFn only avoids LETHAL cells, so a plan could pass within the 0.16 m inscribed ring of an obstacle whose cells arrived late; the local costmap's collision check then tripped mid-drive. Config-only in `nav2_params.yaml` (a `scan` observation source per costmap: `sensor_frame laser`, `data_type LaserScan`, marking+clearing, `obstacle_min_range 0.12` = the LDS floor; local `obstacle_max_range 2.5`/`raytrace 3.0`, global `3.0`/`3.5` — capped to bound raytracing on the 24×24 m @ 1 Hz grid; restart-only, rode a full `deploy.sh robot_bringup web_control` + stack bounce). Verified live: both costmaps log `Using plugin "obstacle_layer"` + `Initialized plugin "obstacle_layer"`, "Managed nodes are active", `ros2 param get … plugins` = `[static_layer, obstacle_layer, inflation_layer]` on both, `/local_costmap` route 200 with inflated-cost cells present, nav container load unchanged (~33% of a core at Nice 10). **Web visibility: the composited master grid already rides the existing `/local_costmap` + `/global_costmap` routes and the Costmap toggle — a new/moved object appears as a yellow→red blob there within ~1 s (the page hint + `mapCost` hover hint updated to say so).** Caveats: a PARKED lidar stops feeding the layer (no scans) — obstacle cells linger until raytraced clear; and the denoise_layer "no obstacle layer to denoise" rejection note is now moot.

- **Recovery motions ZEROED (2026-09-23, user decision) — the per-goal no-recovery-BT toggle was tried and REVERTED the same day.** `recovery_bt.xml`'s recovery branch still runs on a failed goal (clear local+global costmaps → retry Navigate once → abort) but its **BackUp and Spin now carry `backup_dist="0"` / `spin_dist="0"`** — both behaviors complete immediately at zero distance/yaw (Humble's DriveOnHeading/Spin onCycleUpdate succeed the first tick at 0), so the robot NEVER physically backs up or spins during recovery. The toggle that replaced it (web Recovery switch → goals sent as NavigateToPose ACTION goals whose `behavior_tree` field pointed at a no-RecoveryNode `no_recovery_bt.xml`; Humble loads `goal->behavior_tree` per goal while `default_nav_to_pose_bt_xml` is read once at configure — so the goal field is the only restart-free lever) was built, deployed, and board-verified end-to-end, then scrapped for simplicity and fully reverted (git 0db497f + 00a1308, reverts 2ab34ec + 66fc328) — **don't reintroduce the topic-vs-action split lightly**; the stale board copies of `no_recovery_bt.xml` (rsync has no --delete) were rm-ed by hand. To restore real recovery motion: give `backup_dist`/`spin_dist` values in `recovery_bt.xml` + `deploy.sh robot_bringup` (the tree loads at bt_navigator activation only — a nano-nav restart picks it up). Accepted trade: a goal blocked by a persistent obstacle now fails after the single costmap-clear retry with no motion.
- **1 Hz REPLANNING + margin fixes (2026-09-24) — the second "crowds an obstacle then stops" hit was root-caused and fixed with three config levers:** (1) **`RateController hz="1.0"` around ComputePathToPose** in `recovery_bt.xml` — without it ComputePathToPose ran ONCE and the robot followed a STALE plan into the obstacle the obstacle layer had just marked (the actual mid-drive failure mechanism, hit twice that morning); PipelineSequence halts FollowPath for the planning instant each tick (stock default-tree pattern). (2) **`cost_scaling_factor` 3.0 → 1.7 on both costmaps** + RPP `inflation_cost_scaling_factor` 3.0 → 2.0 — steep decay had made cost at 0.17 m tiny so NavFn planned along the inscribed boundary. (3) **local costmap static_layer DROPPED** (local plugins `[obstacle_layer, inflation_layer]`) — an odom-frame rolling window + StaticLayer's map-frame ingest at numeric coords = phantom walls offset by slam's map→odom (measured mild that day, 0.01-0.08 m, but a known 0.3+ m class); the obstacle layer covers live truth. (4) The persisted `nav.json` had drifted BELOW the yaml defaults (inflation 0.20, ⌀ 0.30 — likely old slider drags); corrected to **0.30 / 0.32** via POST /nav/config. All restart-only (rode `deploy.sh robot_bringup`), verified via param readback + "Managed nodes are active".
...
- One-shot deploy: **`scripts/deploy.sh [pkgs…]`** — `rsync`s `src/`+`scripts/` over the
  ...
  The reliable choreography for a bounce on a quiet robot USED TO BE
  ...
  re-parks afterwards. **Superseded 2026-09-24: the `unit_exec.sh nav-loader` LIDAR
  GATE (see the deadlock block) wakes + holds the lidar through the bounce itself —
  a plain `stack.sh restart`/target bounce is again the whole choreography.**
