...
- **Params:** Nav2 tunables live in `config/nav2/nav2_params.yaml` (NOT robot.yaml; NOT live-tunable — restart `nano-nav`). The web `/param` whitelist's `slam_nav` entries are GONE. RPP controller: `rotate_to_heading_angular_vel` is the angular cap (no `max_angular_vel` in Humble); **`controller_frequency` is 4.0 (2026-09-20) — the serial budget: `/cmd_vel` crosses the 115200-baud ESP32 link, which decays under sustained 10 Hz flow (see the HTTP teleop note)**; Ceres threads are hardcoded 50 upstream — the guards are `minimum_travel_distance/heading` pose-graph gating + `CPUAffinity=2 3`/`Nice=10`/`MemoryMax=400M` on `nano-nav.service` + `MALLOC_ARENA_MAX=2`.
...
**Web UI tab/group layout (reorganized 2026-09-16, all IDs unchanged — JS untouched):** the tab bar is **4 tabs**: `Drive` · `AI · Speak` · `Sensors` · `System` (the old separate Speak tab was merged into AI; `data-tab="ai"`/`panel-ai` kept its ids, `panel-speak` is gone). Cards are clustered under `.group` header dividers (styled in `style.css`): **AI · Speak** = 🎙 Speech & voice (TTS, Announce, Face & display) + 🧠 AI & brain (OpenRouter, Personality, Skills, Schedule, Brain purpose, **Brain health** — moved out of Sensors); **Sensors** = 🗺️ SLAM & navigation (Map (Nav2), Locations, Lidar (LDS), Odometry, IMU) + 👁️ Vision (Camera (GPU vision)) + ⚙️ Actuators (Cooling fan, Line lasers, Coprocessor (ESP32)); **System** = Connection, **System health** (moved out of Sensors), View, Health events, Stress test, Power. When adding a card, put it in the right group and re-check the hover-hint cross-references that name other cards/tabs (e.g. the IMU interference hint points at the Actuators card, the stress-test hint at System health).
